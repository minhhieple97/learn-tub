...
const RenderNode: FC<IRenderNodeProps> = ({
  ...
  switch (node.type) {
    ...
    case "paragraph":
      return (
        <p className="mb-2 leading-relaxed">{renderContent(node.content)}</p>
      );
    ...
    case "heading":
      ...
      if (level === 1)
        return <h1 className={headingClass}>{renderContent(node.content)}</h1>;
      if (level === 2)
        return <h2 className={headingClass}>{renderContent(node.content)}</h2>;
      if (level === 3)
        return <h3 className={headingClass}>{renderContent(node.content)}</h3>;
      if (level === 4)
        return <h4 className={headingClass}>{renderContent(node.content)}</h4>;
      if (level === 5)
        return <h5 className={headingClass}>{renderContent(node.content)}</h5>;
      return <h6 className={headingClass}>{renderContent(node.content)}</h6>;

    case "bulletList":
      return (
        <ul className="mb-2 ml-4 list-disc">{renderContent(node.content)}</ul>
      );
