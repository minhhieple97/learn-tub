...
export const uploadScreenshotAction = authAction
  ...
  .action(async ({ parsedInput: { fileData, fileName, fileSize, mimeType }, ctx: { user } }) => {
    ...
    const { error, storagePath } = await uploadScreenshotToStorage(file, profile.id);
...
export const captureAndSaveScreenshotAction = authAction
  ...
  .action(
    async ({
      ...
      // Upload file
      const { error: uploadError, storagePath } = await uploadScreenshotToStorage(file, profile.id);

      if (uploadError) {
        throw new ActionError(`Failed to upload screenshot: ${uploadError.message}`);
      }

      const urlData = await getScreenshotPublicUrl(storagePath);

      // Save metadata
      const { data: metadataResult, error: metadataError } = await saveScreenshotMetadata({
        userId: profile.id,
        videoId,
        fileName,
        fileSize,
      ...
      if (metadataError) {
        throw new ActionError(`Failed to save screenshot metadata: ${metadataError.message}`);
      }

      // Link to note if provided
      if (noteId && metadataResult) {
        const { error: linkError } = await linkScreenshotToNote(noteId, metadataResult.mediaFileId);
        if (linkError) {
          throw new ActionError(`Failed to link screenshot to note: ${linkError.message}`);
        }
      }
...
export const deleteImageAction = authAction
  ...
  .action(async ({ parsedInput: { imageUrl, storagePath }, ctx: { user } }) => {
    ...
    const { error } = await deleteImageFromStorage(imageUrl, storagePath, profile.id);
