...
export function useSubscriptionManagement() {
  const { data: subscriptionData, isLoading } = useQuery(userSubscriptionWithRetryOptions);
  const [isDialogOpen, setIsDialogOpen] = useState(false);

  const subscription = subscriptionData?.subscription;
  const hasActiveSubscription = subscriptionData?.hasActiveSubscription && subscription;
