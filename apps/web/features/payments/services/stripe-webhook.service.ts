...
export class StripeWebhookService {
  ...
  private static async getSubscriptionDataWithPlan(
    customerId: string,
  ...
  private static extractSubscriptionData(subscription: any) {
  ...
  private static async handleSubscriptionCheckout(session: any): Promise<void> {
    ...
    const { hasActivePlan, error: checkError } = await checkUserHasActivePlan(
      userId,
      planId,
    );
    ...
    if (checkError) {
      console.error(
        `❌ Error checking active plan for user ${userId}:`,
        checkError,
      );
      throw new Error(
    ...
    const { data: userSubscription, error: subscriptionError } =
      await upsertUserSubscription(userId, planId, subscriptionData);
    ...
    if (planId !== freePlanId) {
      try {
        const { data: cancelledFreePlan, error: cancelError } =
          await cancelActiveFreePlan(userId, freePlanId);

        if (cancelError) {
          console.error(
            `⚠️ Failed to cancel active free plan for user ${userId}:`,
            cancelError,
          );
        ...
        console.error(
          `⚠️ Error cancelling active free plan for user ${userId}:`,
          error,
        );
      }
    }
  ...
  private static async handleCreditsPurchase(session: any): Promise<void> {
    ...
    const expiresAt = this.calculateExpirationDate(
      CREDIT_EXPIRATION_CONFIG.PURCHASE_DAYS,
    );
    ...
    console.log(
      `💰 ${creditsAmount} credits added to user: ${userId} (expires: ${expiresAt})`,
    );
  }
  ...
  private static async handleInvoicePaymentFailed(invoice: any): Promise<void> {
    ...
    console.log(
      `❌ Subscription payment failed for user: ${userSubscription.user_id}`,
    );
  }
  ...
  private static async handleSubscriptionCreated(
    ...
    if (priceId) {
      ...
      planId = plan?.id || null;
      console.log(
        `🔍 Plan lookup for price ${priceId}:`,
        planId ? "found" : "not found",
      );
    }
    ...
    if (!userId) {
      try {
        ...
        console.log(
          `🔍 Found ${sessions.data.length} checkout sessions for customer`,
        );
        ...
        if (sessionWithMetadata) {
          userId = sessionWithMetadata.metadata?.user_id || null;
    ...
    console.log(
      `🔄 Creating/updating subscription for user ${userId}, plan ${planId}`,
    );
    const subscriptionData = this.extractSubscriptionData(subscription);
    ...
    if (subscription.cancel_at_period_end) {
      ...
      try {
        ...
        if (updateError) {
          ...
          const { error: bucketError } = await markCreditBucketsAsCancelled(
            updatedSubscription.id,
          );

          if (bucketError) {
            console.error(
              "❌ Failed to mark credit buckets as cancelled:",
              bucketError,
            );
          } else {
            console.log(
  ...
  private static async grantMonthlyCredits(
    ...
    const expiresAt = this.calculateExpirationDate(
      CREDIT_EXPIRATION_CONFIG.SUBSCRIPTION_DAYS,
    );
    ...
    if (bucketResult.status === "rejected") {
      throw new Error(
        `Failed to create subscription credit bucket: ${bucketResult.reason}`,
      );
    }
  ...
  private static async handleInvoicePaid(
    ...
    if (invoice.billing_reason !== STRIPE_BILLING_REASON.SUBSCRIPTION_CYCLE) {
      console.log(
        `ℹ️ Skipping invoice.paid for billing_reason: ${invoice.billing_reason}`,
      );
      return;
    }
    ...
    if (!subscriptionId) {
      console.log(
        "ℹ️ Non-subscription invoice paid, skipping renewal processing",
      );
      return;
    }
    ...
    try {
      ...
      if (expireResult.error) {
        throw new Error(
          `Failed to expire current subscription: ${expireResult.error.message}`,
        );
      }

      const { data: newSubscription, error: createError } =
        await createNewUserSubscription(
      ...
      if (createError || !newSubscription) {
        throw new Error(
          `Failed to create new subscription: ${createError?.message}`,
        );
      }
      ...
      if (bucketExpireResult.error) {
        console.error(
          "❌ Failed to expire credit buckets:",
          bucketExpireResult.error,
        );
        ...
        if (transactions.length > 0) {
          parallelOperations.push(
            bulkCreateCreditTransactions(transactions).catch((error) => {
              console.error(
                "❌ Failed to create bulk credit transactions:",
                error,
              );
              return { error };
            }),
      ...
      results.forEach((result, index) => {
        if (result.status === "rejected") {
          console.error(
            `❌ Parallel operation ${index} failed:`,
            result.reason,
          );
        }
      });
