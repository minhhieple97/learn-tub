...
const QuizDashboardQuerySchema = z.object({
  ...
  sortBy: z.enum(['created_at', 'score', 'difficulty']).optional().default('created_at'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryParams = {
      ...
      videoId: searchParams.get('videoId') === 'all' ? undefined : searchParams.get('videoId'),
    ...
    const { search, difficulty, videoId, sortBy, sortOrder, page, limit } = validationResult.data;
